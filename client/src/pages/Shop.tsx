...
export default function Shop() {
  ...
  return <>
    <section className="page-intro shop-intro"><div className="page-rail"><span>01</span><i /></div><div><span className="eyebrow">The counter, today</span><h1>Choose your<br /><em>everyday good.</em></h1></div><div className="page-intro-aside"><p>Daily bakes are ready to order right now. No quote, no guessing—just pick a pickup and we’ll make it happen.</p><button className="text-link" onClick={() => setCartOpen(true)}>View order <span>{itemCount}</span> <ArrowUpRight size={16} /></button></div></section>
    <section className="shop-body"><div className="shop-sticky"><p className="eyebrow">Filter the case</p><div className="category-list">{categories.map((entry) => <button key={entry} className={category === entry ? "selected" : ""} onClick={() => setCategory(entry)}>{entry}<span>{entry === "All" ? bakeryItems.length : bakeryItems.filter((item) => item.category === entry).length}</span></button>)}</div><div className="shop-note"><i /><p><b>Good to know</b><br />We mix a limited number of each bake. Order by 3pm for next-day pickup.</p></div></div>
      <div className="product-grid">{filtered.map((item, index) => <article key={item.id} className={`product-card accent-${item.accent}`}>
        <div className="product-visual"><span className="product-number">0{index + 1}</span><div className={`pastry pastry-${item.id}`} aria-hidden="true"><i /><i /><i /></div>{item.badge && <span className="product-badge">{item.badge}</span>}</div>
